    ...
    a.extend(a.jqx._jqxScheduler.prototype, {
        ...
        _arrangeAutoWidth: function(d) {
            ...
            if (this.width === "auto" || this.width === null || this.autowidth) {
                ...
                width = c;
                if (width + d > this.maxWidth) {
                    this.host.width(this.maxWidth)
                } else {
                    this.host.width(width + d)
                }
                return true
            }
            return false
        ...
        databind: function(h, i, c) {
            ...
            var f = function() {
                if (d == b) {
                    var k = g._resources.length > 1 ? g._resources.length : 1;
                    if (g.resources && g.resources.orientation === "horizontal") {
                        g.tableColumns = k;
                        g.tableRows = 1
                    } else {
                        if (g.resources && g.resources.orientation === "vertical") {
                            g.tableRows = k;
                            g.tableColumns = 1
                        } else {
                            g.tableColumns = 1;
                    ...
                    if (g.autoShowLoadElement && !g._loading) {
                        a(g.dataloadelement).css("visibility", "hidden");
                        a(g.dataloadelement).css("display", "none")
                    }
            ...
            this.dataview.update = function(x) {
                g._loading = false;
                g.appointmentsByKey = new Array();
                g.appointments = new Array();
                g.uiappointments = new Array();
                var C = g.source._source.datafields;
                var n = ["from", "to", "id", "style", "description", "location", "subject", "background", "color", "borderColor", "recurrencePattern", "recurrenceException", "draggable", "resizable", "tooltip", "hidden", "allDay", "timeZone", "ownerId", "resourceId"];
                var l = new Array();
                for (var y = 0; y < g.source.records.length; y++) {
                    var o = g.source.records[y];
                    var m = {};
                    for (var F in g.appointmentDataFields) {
                        var k = g.appointmentDataFields[F];
                        var w = o[k];
                        if (F == "from" || F == "to") {
                            w = new a.jqx.date(w)
                        }
                        if (F == "style") {
                            if (w) {
                                var r = g.getAppointmentColors(w);
                                m.color = r.color;
                                m.background = r.background;
                                m.borderColor = r.border
                            }
                        }
                        if (F == "recurrencePattern") {
                            if (w) {
                                w = new a.jqx.scheduler.recurrencePattern(w);
                                w.timeZone = o.timeZone || g.timeZone
                            }
                        }
                        if (F == "recurrenceException") {
                            var A = new Array();
                            if (w) {
                                if (w.indexOf("EXDATE:") >= 0) {
                                    w = w.substring(w.indexOf("EXDATE:") + 7)
                                }
                                var s = new Array();
                                if (w.indexOf(",") >= 0) {
                                    s = w.split(",")
                                ...
                                for (var z = 0; z < s.length; z++) {
                                    ...
                                    if (v.indexOf(";") >= 0) {
                                        var p = v.split(";")[1];
                                        v = v.split(";")[0];
                                        if (p.toLowerCase().indexOf("display") >= 0 && p.toLowerCase().indexOf("none")) {
                                            m.hidden = true
                                        }
                                    }
                                    try {
                                        var D = a.jqx.scheduler.utilities.untilStringToDate(v);
                                        if (D != "Invalid Date") {
                                            if (m.timeZone) {
                                                D = new a.jqx.date(D, m.timeZone)
                                            } else {
                                                if (g.timeZone) {
                                                    D = D.toTimeZone(g.timeZone)
                                                } else {
                                                    D = new a.jqx.date(D)
                                                }
                                            }
                                        ...
                                        var D = new a.jqx.date(v, g.timeZone)
                                    }
                                    A.push(D)
                                }
                            }
                    ...
                    for (var t in n) {
                        ...
                        if (m[F] == undefined) {
                            ...
                            if (F == "to") {
                                w = new a.jqx.date().addHours(1)
                            }
                            m[F] = w
                        }
                    }
                    m.originalData = o;
                    if (g.resources && !g.resources.source) {
                        if (l.indexOf(m.resourceId) == -1) {
                            l.push(m.resourceId)
                        }
                    }
                    g.appointmentsByKey[o.uid] = m;
                    g.appointments.push(m);
                    var u = new a.jqx.scheduler.appointment(m);
                    if (m.timeZone) {
                        u.from = u.from.toTimeZone(m.timeZone);
                        u.to = u.to.toTimeZone(m.timeZone)
                    }
                    if (g.timeZone) {
                        if (!m.timeZone) {
                            u.timeZone = g.timeZone
                        }
                        u.from = u.from.toTimeZone(g.timeZone);
                        u.to = u.to.toTimeZone(g.timeZone)
                    } else {
                        u.from = u.from.toTimeZone(null);
                        u.to = u.to.toTimeZone(null)
                    }
                    m.jqxAppointment = u;
                    if (m.recurrenceException) {
                        for (var B = 0; B < m.recurrenceException.length; B++) {
                            var D = u.recurrenceException[B];
                            var q = u.clone();
                            q.occurrenceFrom = D;
                            var j = u.duration();
                            if (g.allDay) {
                                j = new a.jqx.timeSpan(10000 * (g.to - g.from))
                            }
                            q.from = D;
                            q.to = D.add(j);
                            q.rootAppointment = u;
                    ...
                    u.boundAppointment = m;
                    u.scheduler = g;
                    g.uiappointments.push(u)
                }
                g._resources = l;
                d++;
                f()
            };
            this.resourcesDataView.update = function(j) {
                if (g.resources.dataField) {
                    g._resources = new Array();
                    for (var k = 0; k < j.length; k++) {
                        if (g._resources.indexOf(j[k][g.resources.dataField]) >= 0) {
                            continue
                        }
                        if (j[k][g.resources.dataField]) {
                            g._resources.push(j[k][g.resources.dataField])
                        }
                    }
                } else {
                    g._resources = j
                }
                if (g.resources.orientation == undefined) {
                    g.resources.orientation = "none"
                }
                d++;
                f()
            };
        ...
        addAppointment: function(n) {
            ...
            if (!n.duration) {
                if (!n.from && !n.to) {
                    ...
                    for (var v in g.appointmentDataFields) {
                        var b = g.appointmentDataFields[v];
                        var p = f[b];
                        if (v == "from" || v == "to") {
                            p = new a.jqx.date(p)
                        }
                        if (v == "style") {
                            if (p) {
                                var k = g.getAppointmentColors(p);
                                d.color = k.color;
                                d.background = k.background;
                                d.borderColor = k.border
                            }
                        }
                        if (v == "recurrencePattern") {
                            if (p) {
                                p = new a.jqx.scheduler.recurrencePattern(p);
                                p.timeZone = f.timeZone || g.timeZone
                            }
                        }
                        if (v == "recurrenceException") {
                            var s = new Array();
                            if (p) {
                                if (p.indexOf("EXDATE:") >= 0) {
                                    p = p.substring(p.indexOf("EXDATE:") + 7)
                                }
                                var l = new Array();
                                if (p.indexOf(",") >= 0) {
                                    l = p.split(",")
                                ...
                                for (var r = 0; r < l.length; r++) {
                                    ...
                                    try {
                                        var t = a.jqx.scheduler.utilities.untilStringToDate(o);
                                        if (t != "Invalid Date") {
                                            if (d.timeZone) {
                                                t = new a.jqx.date(t, d.timeZone)
                                            } else {
                                                if (g.timeZone) {
                                                    t = t.toTimeZone(g.timeZone)
                                                } else {
                                                    t = new a.jqx.date(t)
                                                }
                                            }
                                        ...
                                        var t = new a.jqx.date(o, g.timeZone)
                                    }
                                    s.push(t)
                                }
                            }
                    ...
                    for (var m in e) {
                        ...
                        if (d[v] == undefined) {
                            ...
                            if (v == "to") {
                                p = new a.jqx.date().addHours(1)
                            }
                            d[v] = p
                        }
                    }
                    d.originalData = f;
                    var n = new a.jqx.scheduler.appointment(d);
                    if (d.timeZone) {
                        n.from = n.from.toTimeZone(d.timeZone);
                        n.to = n.to.toTimeZone(d.timeZone)
                    }
                    if (g.timeZone) {
                        if (!d.timeZone) {
                            n.timeZone = g.timeZone
                        }
                        n.from = n.from.toTimeZone(g.timeZone);
                        n.to = n.to.toTimeZone(g.timeZone)
                    } else {
            ...
            var v = g.dataview.generatekey();
            n.id = v;
            g.appointmentsByKey[v] = n;
            g.appointments.push(n);
            var c = {};
            var j = {};
            for (var q in g.appointmentDataFields) {
                var b = g.appointmentDataFields[q];
                var p = n[q];
                c[q] = p;
                if (q == "from" || q == "to") {
                    p = p.toDate()
            ...
            c.originalData = j;
            n.boundAppointment = c;
            c.jqxAppointment = n;
            n.jqxAppointment = n;
            g._raiseEvent("appointmentAdd", {
                appointment: n.boundAppointment
            });
            g.uiappointments.push(n);
            g.changedAppointments[v] = {
                type: "Add",
                appointment: n.boundAppointment
            };
            if (g.hiddenResources && g.hiddenResources[n.resourceId]) {
                g.hideAppointmentsByResource(n.resourceId)
            } else {
                g._renderrows()
            }
            g._selectAppointment(n);
            g._lastSelectedAppointment = n
        },
        ...
        _renderrows: function() {
            ...
            var aM = a.jqx.scheduler.utilities.getEndOfDay(V.getViewEnd());
            V._prepareAppointmentsInView(d, aM);
            var N = new Array();
            if (aE === "weekView" || aE === "dayView") {
                var az = V.getMaxAllDayAppointments(V.appointmentsToRender)
            } else {
                if (aE === "monthView" && G.monthRowAutoHeight && V.rows && V.rows.length == 6) {
            ...
            if (G && G.allDayRowHeight) {
                aP = G.allDayRowHeight
            }
            if (G.timeRuler && G.timeRuler.scale) {
                aK = G.timeRuler.scale
            }
            var aL = 0;
            V.table[0].rows = new Array();
            V.rows = new Array();
            var a4 = V.toTP("jqx-cell") + " " + V.toTP("jqx-widget-content");
            if (V.rtl) {
                a4 += " " + V.toTP("jqx-cell-rtl")
            }
            ...
            V.focusedCell = null;
            var E = new Array();
            var aC = V.source.records;
            aC = V.dataview.evaluate(aC);
            V.dataViewRecords = aC;
            V.cells = new Array();
            var E = V.dataViewRecords;
            V.renderedRecords = V.getRows();
            var aY = V.tableZIndex;
            ...
            if (W) {
                for (var X = 0; X < b; X++) {
                    ...
                    var aI = a('<table><tr><td role="gridcell" style="max-width: ' + aJ + "px; width:" + aJ + 'px;" class="' + a4 + '"></td></tr></table>');
                    a(document.body).append(aI);
                    var at = aI.find("td");
            ...
            var a2 = 1;
            if (aE === "timelineDayView" || aE === "timelineWeekView") {
                b = aR * b;
                a2 = aR;
                av = b / V.tableColumns
            } else {
                if (aE === "timelineMonthView") {
                    av = b / V.tableColumns
                } else {
                    if (aE === "monthView") {
                        if (G.showWeekNumbers) {
            ...
            for (var X = 0; X < b; X++) {
                ...
                if (aE == "monthView" && V.rtl && ac.timeColumn) {
                    aJ++
                }
                if (W) {
                    var P = aJ;
                    ...
                    H += "<col style='max-width: " + aJ + "px; width: " + aJ + "px;'>"
                }
                aG += aJ
            }
            H += "</colgroup>";
            ...
            if (ai === 0) {
                ...
                var aI = '<td colspan="' + V.columns.records.length + '" role="gridcell" style="border: none; min-height: ' + z + "px; height: " + z + "px; max-width:" + aG + "px; width:" + aG + "px;";
                var a4 = V.toTP("jqx-cell") + " " + V.toTP("jqx-grid-cell") + " " + V.toTP("jqx-item");
                if (V.rtl) {
                    a4 += " " + V.toTP("jqx-cell-rtl")
                }
                a4 += " " + V.toTP("jqx-center-align");
                aI += '" class="' + a4 + '">';
                aI += V.schedulerLocalization.emptyDataString;
                aI += "</td>";
                u += aI;
                H += u;
                V.table[0].style.width = aG + 2 + "px";
                aL = aG
            }
            var au = V.getRows();
            var d = V.getViewStart();
            ...
            var a1 = 0;
            if (!V.columnGroups) {
                a1 += V.showHeader ? V.columnsHeight : 0
            } else {
                a1 += V.showHeader ? V.columnsheader.height() : 0
            }
            if (V.showToolbar) {
                a1 += V.toolbarHeight
            }
            ...
            if (B == null || B == "auto") {
                ...
                if (V.tableRows == 1) {
                    ...
                    if (V.columns.records.length > 10) {
                        B = z > 0 ? (z - V.scrollBarSize - 5 - a1) : p
                    }
                } else {
                    B = z > 0 ? (z - a1 - m) / (ai) : p;
                    if (V.columns.records.length > 10) {
                        B = z > 0 ? (z - V.scrollBarSize - 5 - a1 - ag * m) / (ag * ai) : p
                    }
                }
                if (aE === "dayView" || aE === "weekView") {
                    if (f) {
                        if (aP == null || aP == "auto") {
                            B = z > 0 ? (z - a1) / (V.tableRows * (ai)) : ap;
                            aP = B
                        } else {
                            B = z > 0 ? (z - a1 - aP - ag * m) / (ag * (ai - 1)) : p
                        }
                    }
                } else {
                    if (aE === "monthView") {
                ...
                aP = Math.max(ap, aP);
                V._allDayRowHeight = aP;
                V._defaultRowHeight = p;
                aP = az * (p - 2);
                aP = Math.max(ap, 17 + aP);
                V._allDayRowFullHeight = aP;
                if (G.allDayRowHeight) {
                    V._allDayRowHeight = G.allDayRowHeight;
                    aP = az * (G.allDayRowHeight - 2);
                    V._allDayRowFullHeight = aP
                }
            }
            var aB = new Array();
            if (V.resources && V.resources.colorScheme) {
                ...
                while (V._resources.length > aB.length - 1) {
                    var aV = parseInt(V.resources.colorScheme.substring(6));
                    if (aV >= 27) {
                        aV = 0
                    }
                    aB = aB.concat(V.colorSchemes[aV].colors);
                    aV++
                }
            }
            for (var aD = 0; aD < V.tableRows; aD++) {
                var aq = 0;
                var S = 0;
                var e = 23;
                ...
                var ar = new Array();
                var k = aE != "monthView" && aE != "timelineDayView" && aE != "timelineWeekView" && aE != "timelineMonthView" && f ? 1 : 0;
                if (V.tableRows > 1) {
                    ...
                    if (!W) {
                        a4 += " " + V.toTP("jqx-grid-cell-pinned");
                        Z += ' colspan="' + b + '"';
                        var P = 0;
                        for (var Q = 0; Q < b; Q++) {
                            var ab = g(Q);
                    ...
                    var aI = '<td style="height:' + m + "px;";
                    if (X == b - 1 && b == 1 && !V.rtl) {
                        aI += "border-right-color: transparent;"
                    }
                    if (V._resources.length > 0 && V.resources.colorScheme && V.resources.orientation != "none") {
                        ...
                        aN = aB[aD];
                        aN = V.hexToRgba(aB[aD], 0.7).toString();
                        D = L(V.hexToRgba(aB[aD], 0.7));
                        y = aB[aD];
                        aI += "background: " + aN + ";";
                        aI += "border-color: " + y + ";";
                        aI += "color: " + D + ";"
                    }
                    aI += '" class="' + a4 + '" role="gridcell" ' + Z + "";
                    var aW = V._resources[aD];
                    aI += "><span style='position: relative;'>" + aW + "</span>";
                    aI += "</td>";
                    u += aI;
                    H += u
                }
                for (var Y = 0; Y < ai; Y++) {
                    ...
                    for (var X = 0; X < b; X++) {
                        ...
                        if (!ar[X]) {
                            if (!V.rtl) {
                                if (aE.toString().indexOf("timelineDayView") >= 0) {
                                    var I = X;
                                    ar[X] = A.addHours(S).addMinutes(I * an, false)
                                } else {
                                    if (aE.toString().indexOf("timelineWeekView") >= 0) {
                                        ar[X] = A.addDays(aT).addHours(S, false).addMinutes(aw * an, false)
                                    } else {
                                ...
                                if (aE.toString().indexOf("timelineDayView") >= 0) {
                                    ...
                                    if (aE.toString().indexOf("timelineWeekView") >= 0) {
                                        if (G.showWeekends || G.showWeekends == undefined) {
                                            ar[X] = A.addDays(6 - aT).addHours(1 + e, false).addMinutes(-aw * an, false)
                                        } else {
                                            ar[X] = A.addDays(4 - aT).addHours(1 + e, false).addMinutes(-aw * an, false)
                                        }
                                    } else {
                                        if (aE == "dayView" || aE == "monthView" || aE == "weekView") {
                                            if (G.showWeekends || G.showWeekends == undefined) {
                        ...
                        if (!R.timeColumn && (aE === "dayView" || aE === "weekView")) {
                            ax++
                        } else {
                            if (!R.timeColumn && (aE === "monthView" && G.showWeekNumbers)) {
                                ax++
                            } else {
                                if ((aE === "monthView" && !(G.showWeekNumbers)) || aE === "timelineMonthView") {
                                    ax++
                                } else {
                                    if (aE == "timelineDayView" || aE == "timelineWeekView") {
                                        ax++
                        ...
                        if (V.sortcolumn === R.displayfield) {
                            a4 += " " + V.toTP("jqx-grid-cell-sort")
                        }
                        if (V.altRows && Y % 2 != 0) {
                            a4 += " " + V.toTP("jqx-grid-cell-alt")
                        }
                        if (Y == 0 && f && (aE === "dayView" || aE === "weekView")) {
                            a4 += " " + V.toTP("jqx-grid-cell-alt")
                        }
                        if (C > V.max || C < V.min) {
                            a4 += " " + V.toTP("jqx-scheduler-disabled-cell")
                        }
                        if (V.rtl) {
                            a4 += " " + V.toTP("jqx-cell-rtl")
                        }
                        ...
                        var aI = '<td data-key="' + am + '" data-view="' + (aF) + '" data-date="' + C.toString() + '" role="gridcell" ' + Z + ' style="max-width:' + aJ + "px; width:" + aJ + "px;  height:" + B + "px;";
                        if (Y == 0 && f && (aE === "dayView" || aE === "weekView")) {
                            var aI = '<td  data-key="' + am + '" data-view="' + (aF) + '" data-end-date="' + a.jqx.scheduler.utilities.getEndOfDay(C).toString() + '" data-date="' + C.clearTime().toString() + '" role="gridcell" ' + Z + ' style="max-width:' + aJ + "px; width:" + aJ + "px; height:" + aP + "px;"
                        }
                        if (aE == "monthView" && G.monthRowAutoHeight && N.length > 0) {
                            if (N[Y] && N[Y] != "auto") {
                                if (B < N[Y]) {
                                    var aI = '<td data-key="' + am + '" data-view="' + (aF) + '" data-date="' + C.toString() + '" role="gridcell" ' + Z + ' style="max-width:' + aJ + "px; width:" + aJ + "px;  height:" + N[Y] + "px;"
                                }
                            }
                        }
                        if (V.resources && V._resources.length > 0 && V.resources.colorScheme && V.resources.orientation !== "none") {
                            ...
                            aN = aB[aD + ae - 1];
                            if (!(Y == 0 && f && (aE === "dayView" || aE === "weekView"))) {
                                aN = V.hexToRgba(aB[aD + ae - 1], 0.3).toString()
                            } else {
                                aN = V.shadeColor(aB[aD + ae - 1], 0.3).toString()
                            }
                            if (!(Y == 0 && f && (aE === "dayView" || aE === "weekView"))) {
                                if (aE.indexOf("month") == -1) {
                                    if (!R.timeColumn) {
                                        if (G.showWorkTime !== false) {
                                            ...
                                            if (C.dayOfWeek() >= O && C.dayOfWeek() <= J) {
                                                if (C.hour() >= ay && C.hour() < l) {
                                                    aN = V.hexToRgba(aB[aD + ae - 1], 0.1).toString()
                                                }
                                            }
                                        }
                                    }
                        ...
                        if (R.timeColumn && !R.hidden && aE === "monthView") {
                            var a4 = V.toTP("jqx-scheduler-time-column") + " " + V.toTP("jqx-widget-header") + " " + V.toTP("jqx-cell") + " " + V.toTP("jqx-grid-cell") + " " + V.toTP("jqx-item");
                            a4 += " " + V.toTP("jqx-scheduler-week-number-column");
                            if (V.tableRows > 1) {
                                var a4 = V.toTP("jqx-cell") + " " + V.toTP("jqx-grid-cell") + " " + V.toTP("jqx-item")
                            }
                            if (V.rtl) {
                            ...
                            if (C.minute() === 0) {
                                var aI = '<td classname="jqx-disableselect" data-time-slot="true" data-date="' + C.toString() + '" role="gridcell" style="max-width:' + aJ + "px; width:" + aJ + "px;";
                                if (X == b - 1 && b == 1 && !V.rtl) {
                                    aI += "border-right-color: transparent;"
                                } else {
                                    if (V.rtl && X == 0) {
                                        aI += "border-left-color: transparent;"
                                    }
                                }
                                if (X == b - 1 && V.rtl) {
                                    aI += "border-right-color: transparent;"
                        ...
                        if (R.timeColumn && !R.hidden && (aE === "dayView" || aE === "weekView")) {
                            var a4 = V.toTP("jqx-scheduler-time-column") + " " + V.toTP("jqx-widget-header") + " " + V.toTP("jqx-cell") + " " + V.toTP("jqx-grid-cell") + " " + V.toTP("jqx-item");
                            if (V.tableRows > 1) {
                                var a4 = V.toTP("jqx-cell") + " " + V.toTP("jqx-grid-cell") + " " + V.toTP("jqx-item")
                            }
                            ...
                            if (Y > 0) {
                                if (C.minute() === 0) {
                                    var aI = '<td classname="jqx-disableselect" data-time-slot="true" data-date="' + C.toString() + '" rowspan=' + aR + ' role="gridcell" ' + Z + ' style="max-width:' + aJ + "px; width:" + aJ + "px;";
                                    if (X == b - 1 && b == 1 && !V.rtl) {
                                        aI += "border-right-color: transparent;"
                                    } else {
                                        if (V.rtl && X == 0) {
                                ...
                                if (Y == 0 && f && (aE === "dayView" || aE === "weekView")) {
                                    var aI = '<td data-view="' + (aF) + '" data-time-slot="true" data-end-date="' + a.jqx.scheduler.utilities.getEndOfDay(C).toString() + '" data-date="' + C.toString() + '" role="gridcell" ' + Z + ' style="max-width:' + aJ + "px; width:" + aJ + "px;"
                                }
                                if (X == b - 1 && b == 1 && !V.rtl) {
                                    aI += "border-right-color: transparent;"
                                } else {
                            ...
                            if (Y > 0) {
                                ...
                                if (V.rtl && X == 0 && aE == "monthView" && G.showWeekNumbers) {
                        ...
                        if (!R.timeColumn) {
                            if (V.rowinfo[aH.uid]) {
                                if (V.rowinfo[aH.uid].selected) {
                                    if (V.editKey !== aH.uid) {
                                        if (V.selectionMode !== "none") {
                                            a4 += " " + V.toTP("jqx-grid-cell-selected");
                                            a4 += " " + V.toTP("jqx-fill-state-pressed");
                                            a4 += " " + V.toTP("jqx-scheduler-cell-selected")
                                        }
                                    }
                                }
                            }
                        ...
                        if (!(Y == 0 && f && (aE === "dayView" || aE === "weekView"))) {
                            if (aE.indexOf("month") == -1) {
                                a4 += " " + V.toTP("jqx-grid-cell-nowrap");
                                if (Y % 2 == 1 && !R.timeColumn) {
                                    a4 += " " + V.toTP("jqx-scheduler-middle-cell")
                                }
                                if (!R.timeColumn) {
                                    if (G.showWorkTime !== false) {
                                        var O = 1;
                                        ...
                                        if (C.dayOfWeek() >= O && C.dayOfWeek() <= J) {
                                            if (C.hour() >= ay && C.hour() < l) {
                                                a4 += " " + V.toTP("jqx-scheduler-work-time-cell")
                                            } else {
                                                a4 += " " + V.toTP("jqx-scheduler-not-work-time-cell")
                                            }
                                        } else {
                                            a4 += " " + V.toTP("jqx-scheduler-not-work-time-cell")
                                        }
                        ...
                        if (aE === "monthView") {
                            if (!V.touchDevice && !R.timeColumn) {
                                a4 += " " + V.toTP("jqx-top-align")
                            }
                            var af = "dd";
                            ah = C.toDate();
                            var a0 = V.toTP("jqx-scheduler-month-outer-cell") + " ";
                            ...
                            if (V.touchDevice) {
                                if (B > 16) {
                                    ah = "<div class='" + a0 + V.toTP("jqx-scheduler-month-cell-touch") + "'>" + a.jqx.dataFormat.formatdate(ah, af, V.schedulerLocalization) + "<span style='display: none; float: right; cursor: pointer; width:16px; height: 16px;' class='" + V.toTP("jqx-icon-arrow-down") + "'></span></div>"
                                } else {
                                    ah = "<div class='" + a0 + V.toTP("jqx-scheduler-month-cell-touch") + "'>" + a.jqx.dataFormat.formatdate(ah, af, V.schedulerLocalization) + "<span style='display: none; float: right; cursor: pointer;' class='" + V.toTP("jqx-icon-arrow-down") + "'></span></div>"
                                }
                            } else {
                                if (B > 16) {
                                    ah = "<div class='" + a0 + V.toTP("jqx-scheduler-month-cell") + "'>" + a.jqx.dataFormat.formatdate(ah, af, V.schedulerLocalization) + "<span style='display: none; float: right; cursor: pointer; width:16px; height: 16px;' class='" + V.toTP("jqx-icon-arrow-down") + "'></span></div>"
                                } else {
                                    ah = "<div class='" + a0 + V.toTP("jqx-scheduler-month-cell") + "'>" + a.jqx.dataFormat.formatdate(ah, af, V.schedulerLocalization) + "<span style='display: none; float: right; cursor: pointer;' class='" + V.toTP("jqx-icon-arrow-down") + "'></span></div>"
                                }
                            }
                        } else {
                            if (aE === "timelineWeekView" || aE === "timelineDayView" || aE === "timelineMonthView") {
                                a4 += " " + V.toTP("jqx-right-align jqx-bottom-align");
                                ah = "<span style='display: none; float: right; cursor: pointer; width:16px; height: 16px;' class='" + V.toTP("jqx-icon-arrow-down") + "'></span>"
                            }
                        }
                        if (Y == 0 && f && (aE === "dayView" || aE === "weekView") && !R.timeColumn) {
                            a4 += " " + V.toTP("jqx-top-align");
                            a4 += " " + V.toTP("jqx-scheduler-all-day-cell");
                            if (V.rtl) {
                                a4 += " " + V.toTP("jqx-rtl")
                            }
                            ah = C.toDate();
                        ...
                        if (R.timeColumn && !R.hidden) {
                            ...
                            if (Y < k) {
                                if (!V.rtl) {
                                    a4 += " " + V.toTP("jqx-scheduler-time-column-header-cell")
                                } else {
                                    a4 += " " + V.toTP("jqx-scheduler-time-column-header-cell-rtl")
                                }
                                ah = ""
                            }
                            if (Y < k) {
                                if (R._text) {
                                    a4 += " " + V.toTP("jqx-bottom-align")
                                }
                        ...
                        if (R.cellclassname != "" && R.cellclassname) {
                            if (typeof R.cellclassname == "string") {
                                ...
                                var aZ = R.cellclassname(Y, R.datafield, ah, aH);
                                if (aZ) {
                                    a4 += " " + aZ
                                }
                            }
                        ...
                        if (R.cellsRenderer != "" && R.cellsRenderer) {
                            var aS = au.indexOf(aH);
                            ah = R.cellsRenderer(aS, R.datafield, ah, aH)
                        }
                        aI += '" class="' + a4 + '">';
                        aI += ah;
                        aI += "</td>";
                    ...
                    if (!V.rowinfo[aH.uid]) {
                        ...
                        if (!("row" in V.rowinfo[aH.uid])) {
                            V.rowinfo[aH.uid].row = aH
                        }
                    }
                }
            }
            ...
            var aQ = a.jqx.browser.msie && a.jqx.browser.version < 9;
            var aO = null;
            if (f && (aE === "dayView" || aE === "weekView") && V.tableRows == 1) {
            ...
            var aU = a(V.pinnedtable[0].firstChild);
            V._table = Q;
            V._pinnedtable = aU;
            if (a.jqx.browser.msie && a.jqx.browser.version < 10) {
                V._table[0].style.width = aL + "px";
                V._pinnedtable[0].style.width = aL + "px"
            }
            if (ai === 0) {
                V._table[0].style.width = (2 + aL) + "px";
                V._pinnedtable[0].style.width = (2 + aL) + "px"
            }
            ...
            for (var Y = 0; Y < U; Y++) {
                ...
                aH.belowRow = null;
                if (V.rows.length > 0) {
                    aH.aboveRow = V.rows[V.rows.length - 1];
                    V.rows[V.rows.length - 1].belowRow = aH
                }
                aH.element = v;
                V.rows.push(aH);
                for (var X = 0; X < v.cells.length; X++) {
                    ...
                    if (!V.columnCells[aH.cells.length]) {
                        V.columnCells[aH.cells.length] = new Array()
                    }
                    V.columnCells[aH.cells.length].push(M);
                    if (V.cellWidth == 0) {
                    ...
                    if (V._views[V._view].type == "monthView") {
                        if (aH.top == undefined) {
                            aH.top = parseInt(a(M).position().top)
                        }
                    }
                    aH.cells.push(M);
                    V.rowByCell[M.getAttribute("data-key")] = aH
                }
            }
            if (ai === 0) {
                V._table[0].style.tableLayout = "auto"
            }
            V._renderAppointments(d, aM);
            if (V.pinnedtable[0].style.display != "none" || ((aE == "dayView" || aE == "weekView") && V.tableRows > 1)) {
                V._updateScrollbars(V.host.height())
            }
            if (aE === "monthView" && G.monthRowAutoHeight) {
                if (N.length == 0) {
                    V._renderrows()
                } else {
                    V._updateScrollbars(V.host.height())
        ...
        _renderMonthAppointments: function() {
            ...
            var A = new Array();
            var S = this.rows;
            var O = a(".jqx-scheduler-month-cell:first").height();
        ...
        _getMonthAppointmentsPerWeek: function() {
            ...
            var C = new Array();
            var U = this.rows;
            var Q = a(".jqx-scheduler-month-cell:first").height();
            if (!M.rows) {
                return new Array()
            }
            for (var e = 0; e < this.appointmentsToRender.length; e++) {
        ...
        _rendercolumnheaders: function() {
            ...
            a.each(this.columns.records, function(D, I) {
                ...
                if (this.width.toString().indexOf("%") != -1 || this._percentagewidth != undefined) {
                    if (this._percentagewidth != undefined) {
                        J = parseFloat(this._percentagewidth) * m / 100
                    } else {
                        J = parseFloat(this.width) * m / 100
                    }
                    J = Math.round(J);
                    K = true
                }
                if (this.width != "auto" && !this._width && !K) {
                    if (J < this.minwidth && this.minwidth != "auto") {
                        J = this.minwidth
                    }
                    if (J > this.maxwidth && this.maxwidth != "auto") {
                        J = this.maxwidth
                    }
                    z[0].style.width = parseInt(J) + "px"
                } else {
                    if (K) {
                        if (J < this.minwidth && this.minwidth != "auto") {
                            J = this.minwidth
